...
export class SchoolServiceImpl implements SchoolService {
  ...
  public async delete(id: number) {
    try {
      ...
      if (eMessage.indexOf('registration_school_id_fkey') > -1) {
        ret.message = '请先删除报名中包含此学校信息的记录'
      }

      if (eMessage.indexOf('enrollment_school_id_fkey') > -1) {
        ret.message = '请先删除招生中包含此学校信息的记录'
      }
